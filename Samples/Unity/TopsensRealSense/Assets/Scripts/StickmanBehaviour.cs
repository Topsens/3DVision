...
public class StickmanBehaviour : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (null != Camera.main)
        {
            this.people = Camera.main.GetComponent<PeopleBehaviour>();
            if (null != this.people)
            {
                this.people.UsersReady += this.OnUsers;
            }
        }
    }
    ...
    private void OnUsers(UsersFrame frame)
    {
        lock (this)
        {
    ...
    private PeopleBehaviour people;
